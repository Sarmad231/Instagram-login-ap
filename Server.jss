...
// Database Connection (Render.com se milega)
const pool = new Pool({
...
app.post("/login", async (req, res) => {
    ...
    try {
        ...
        if (user && bcrypt.compareSync(password, user.password)) {
            res.send(`<style>body{font-family: sans-serif; text-align: center; padding-top: 50px;}</style><h1>Welcome, ${user.username}!</h1><p>This is a real, working login system on Render.</p><a href="/">Log out</a>`);
        } else {
